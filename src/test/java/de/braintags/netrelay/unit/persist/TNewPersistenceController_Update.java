 ...
 * 
 */
public class TNewPersistenceController_Update extends AbstractPersistenceControllerTest {
  private static final io.vertx.core.logging.Logger LOGGER = io.vertx.core.logging.LoggerFactory
      .getLogger(TNewPersistenceController_Update.class);
  ...
  public void testUpdateAsParameter(TestContext context) {
    ...
    try {
      String url = "/products/update2.html?" + createReferenceAsParameter(context, Action.UPDATE, mapper);
      testRequest(context, HttpMethod.POST, url, req -> {
        Buffer buffer = Buffer.buffer();
        buffer.appendString("origin=junit-testUserAlias&login=admin%40foo.bar&pass+word=admin");
        buffer.appendString("&").appendString(NetRelayExt_FileBasedSettings.SIMPLEMAPPER_NAME)
