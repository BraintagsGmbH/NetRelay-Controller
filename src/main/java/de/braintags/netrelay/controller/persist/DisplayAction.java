...
public class DisplayAction extends AbstractAction {
   ...
   * 
   */
  public DisplayAction(PersistenceControllerNew persitenceController) {
  ...
  private void addQueryCritera(IQuery<?> query, CaptureMap map) {
    if (map.containsKey(PersistenceControllerNew.SELECTION_SIZE_CAPTURE_KEY)) {
      query.setLimit(Integer.parseInt(map.get(PersistenceControllerNew.SELECTION_SIZE_CAPTURE_KEY)));
    }
    if (map.containsKey(PersistenceControllerNew.SELECTION_START_CAPTURE_KEY)) {
      query.setStart(Integer.parseInt(map.get(PersistenceControllerNew.SELECTION_START_CAPTURE_KEY)));
    }
    if (map.containsKey(PersistenceControllerNew.ORDERBY_CAPTURE_KEY)) {
      addSortDefintions(query, map);
    }
  }

  private void addSortDefintions(IQuery<?> query, CaptureMap map) {
    String str = map.get(PersistenceControllerNew.ORDERBY_CAPTURE_KEY);
